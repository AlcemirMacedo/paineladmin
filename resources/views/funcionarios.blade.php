    ...
    <div class="dash-fornecedor">
        <div class="pesq-field">
            <form action="/searchrecibo" class="form-inline" method="GET">
                <a href="/cadastrofuncionario" class="btn btn-success" style="margin-right: 10px">Cadastrar Funcionário</a>
                ...
                @foreach ($sql as $item)
                    <form action="#" method="POST" onsubmit="showLoader()">
                        ...
                        <tr>
                            ...
                            <td class="desc_forn">{{ $item->matricula_funcionario }}<input title="{{ $item->matricula_funcionario }}" type="hidden" placeholder="{{ $item->matricula_funcionario }}" value="{{ $item->matricula_funcionario}}" name="matricula" readonly></td>
                            <td class="name_forn">{{ $item->cpf_funcionario }}<input type="hidden" placeholder="{{ $item->cpf_funcionario }}" value="{{ $item->cpf_funcionario }}" name="nome" readonly></td>
                            <td>{{ $item->nome_funcionario}}<input type="hidden" placeholder="{{ $item->nome_funcionario }}" value="{{ $item->nome_funcionario }}" name="numero" readonly></td>
                            <td>{{ $item->cargo_funcionario }}<input type="hidden" placeholder="{{ $item->cargo_funcionario }}"  value="{{ $item->cargo_funcionario }}" name="cpfcnpj" readonly></td>
                            <td>{{ $item->contato_funcionario }}<input type="hidden" placeholder="{{ $item->contato_funcionario }}" value="{{ $item->contato_funcionario }}" name="valor" readonly></td>
                            ...
                            <td>
                                ...
                                <a class="excluir-bot" title="Excluir" href="/exluirrecibo/{{ $item->id_funcionario }}">
